...
def get_image_code(image_code_id):
    ...
    # 返回图片
    resp = make_response(image_data)  # btyte
...
def get_sms_code(mobile):
    ...
    # ret = result_obj.get(timeout=1)
    # print("ret=",ret)
    # 发送成功
    return jsonify(errno=RET.OK,errmsg="发送成功")
print(1)
# import json
# from ihome.utils.sms_code import get_code
# from ihome.utils.sms_code import send_sms
# from ihome.utils import sms_code
# # @api.route("/sms_codes/<re(r'1[34578]\d{9}'):mobile>")
# @api.route("/sms_codes/<re(r'1[35689]\d{9}'):mobile>")
# def get_sms_code(mobile):
#    # 获取两个参数
#    image_code = request.args.get('image_code')# 用户输入的图片验证码
#    image_code_id = request.args.get('image_code_id')  # 图片验证码UUID
#
#    # 判断完整性
#    if not all([image_code,image_code]):
#       return jsonify(errno = RET.PARAMERR,errmsg='参数不完整')
#    # 从redis中根据image_code_id 找真实图片文本
#    try:
#       real_image_code = str(redis_storage.get("image_code_%s"%image_code_id),encoding='utf-8')
#       print('真实图片验证码',real_image_code)
#       # 判断是否过期
#       if real_image_code is None:
...
#    # 1.判断是否已经发送
#    try:
#       is_fasong = redis_storage.get('send_sms_code_%s'%mobile)
#    except Exception as e:
#       current_app.logger.error(e)
#    else:
#       if is_fasong is not None:
#          return jsonify(errno=RET.REQERR, errmsg='已发过短信，60秒后重发')
#
#    # 2.判断手机号是否存在（存在是老用户，不发短信
#    try:
#       user = User.query.filter_by(mobile=mobile).first()
#       print('手机对应的用户:',user)
#    except Exception as e:
...
#
#    # 3.如果是新用户，新手机号，生成验证码，发短信
#    sms_code = get_code(6,False)
#    print('短信验证码',sms_code)
#
#    # 4.存在redis中，60s有效
#    try:
#       # 保存60s
#       redis_storage.setex("send_code_%s"%mobile,constants.SMS_CODE_REDIS_EXPIRES,sms_code)
#       # 存储是否发送标记！ 60s之后自动消失   “1”是任意写的
#       redis_storage.setex("send_sms_code_%s"%mobile,constants.SMS_CODE_REDIS_EXPIRES,1)
#    except Exception as e:
#       current_app.logger.error(e) # 有异常存储日志
#       return jsonify(errno=RET.DBERR, errmsg='保存验证码错误')
#
#    result = send_sms(mobile,sms_code)
#    if json.loads(result).get("Code")=="OK":
#       return jsonify(errno = RET.OK,errmsg = "发送成功")
#    else:
#       current_app.logger.error(result)  # 有异常存储日志
#       return jsonify(errno=RET.OK, errmsg='发送失败')
