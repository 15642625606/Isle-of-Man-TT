...
from ihome import db, constants, redis_storage
...
def get_area_info():
    ...
    # 尝试从redis中读取数据
    try:
        resp_json = redis_storage.get("area_info")
    ...
    # 将数据保存到redis中
    try:
        redis_storage.setex("area_info", constants.AREA_INFO_REDIS_CACHE_EXPIRES, resp_json)
    except Exception as e:
        current_app.logger.error(e)
...
def save_house_info():
    ...
    print(title,price,area_id,address,room_count,acreage,unit,capacity,beds,deposit,min_days,max_days)
    ...
    if area is None:
        return jsonify(errno=RET.NODATA,errmsg='房区信息有误')
...
def get_house_index():
    ...
    # 从缓存中尝试获取数据
    try:
        ret = redis_storage.get("home_page_data")
    except Exception as e:
        current_app.logger.error(e)
        ret = None
    ...
    if ret:
        ...
        try:
            redis_storage.setex("home_page_data", constants.HOME_PAGE_DATA_REDIS_EXPIRES, json_houses)
        except Exception as e:
            current_app.logger.error(e)
...
def get_house_detail(house_id):
    ...
    # 先从redis缓存中获取信息
    try:
        ret = redis_storage.get("house_info_%s" % house_id)
    except Exception as e:
        current_app.logger.error(e)
        ret = None
    if ret:
    ...
    try:
        redis_storage.setex("house_info_%s" % house_id, constants.HOUSE_DETAIL_REDIS_EXPIRE_SECOND, json_house)
    except Exception as e:
        current_app.logger.error(e)
...
def get_house_list():
    ...
    try:
        resp_json = redis_storage.hget(redis_key, page)
    except Exception as e:
        current_app.logger.error(e)
    else:
        if resp_json:
